...
// --- load guard & version banner ---
(function(){
  const VER = "v9.61-email-styles+G62+zebra+note-fix";
  if (window.__poly_cfg_loaded__) {
    console.warn("Poly Configurator script already loaded; skipping re-execution.", VER);
    return;
  }
  window.__poly_cfg_loaded__ = true;
  console.log("Poly Configurator script loaded:", VER);
})();
...
async function init() {
  try {
    ...
    const camWrap = select("cameraChoice", "Select Camera (for G62 very large rooms)", [
      { value: "None", label: "None" },
      { value: "E60", label: "Poly E60 (9W1A6AA#AC3)" },
      { value: "E70", label: "Poly E70 (842F8AA)" }
    ]);
    ...
    // Delegated click handler for Copy BOM (HTML)
    resultDiv.addEventListener('click', async (ev) => {
      const tgt = ev.target;
      if (!tgt || tgt.id !== 'copyHtmlBtnV2') return;
      try {
        console.log('[BOM] Copy button clicked');
        const bomNode = document.getElementById('bomCopy');
        if (!bomNode) throw new Error('BOM container not found');
        // Prefer robust fallback first for widest compatibility
        const range = document.createRange();
        range.selectNodeContents(bomNode);
        ...
        // Then try Clipboard API for rich HTML if available
        if (navigator.clipboard && window.ClipboardItem) {
          const html = bomNode.innerHTML;
          const text = bomNode.innerText;
          const data = {
        ...
        tgt.textContent = execOk ? 'Copied!' : 'Copied (API)!';
        setTimeout(() => (tgt.textContent = 'Copy BOM (HTML)'), 1200);
      } catch (err) {
        console.error('[BOM] Copy failed', err);
        tgt.textContent = 'Copy failed';
        setTimeout(() => (tgt.textContent = 'Copy BOM (HTML)'), 1500);
      }
    });
    ...
    function updateExpansionMicOptions() {
      ...
      // Update helper note text dynamically for V12 Small
      const smallV12 = ((typeOfSystem === "USB bar only" || typeOfSystem === "Windows PC based solution") && roomSize === "Small");
      if (smallV12) {
        info.textContent = "Note: IP table/ceiling mics are not supported with V12, X32, X52, or V52. Small room (V12) only supports Qty = 1 A2 mic pod";
      } else {
        info.textContent = "Note: IP table/ceiling mics are not supported with V12, X32, X52, or V52. Use Analog or A2 mics.";
      }
...
function updateA2Qty() {
    ...
    function updateCameraVisibility(){
      ...
      const show = (typeOfSystem === "Android appliance based solution" && roomSize === "Very large");
    ...
    function generate(catalog) {
      ...
      let html = `
<div class="mb-2">
  <button id="copyHtmlBtnV2" type="button" class="px-3 py-1 border rounded">Copy BOM (HTML)</button>
</div>
<div id="bomCopy" contenteditable="true" style="outline:none;">
<p class='font-bold mb-2'>Your BOM</p>
<p class='text-sm italic text-gray-600 mb-2'>Disclaimer: Generated with AI, subject to errors. Not an official HP tool.</p>
<table style="border-collapse:collapse;width:100%;font-size:14px;table-layout:auto;border:1px solid #e5e7eb;">
<thead>
  <tr>
    <th style="text-align:left;border:1px solid #e5e7eb;padding:8px;background:#f3f4f6;">Qty</th>
    <th style="text-align:left;border:1px solid #e5e7eb;padding:8px;background:#f3f4f6;">SKU</th>
    <th style="text-align:left;border:1px solid #e5e7eb;padding:8px;background:#f3f4f6;">Description</th>
    ${includePrices ? '<th style="text-align:left;border:1px solid #e5e7eb;padding:8px;background:#f3f4f6;">MSRP</th>' : ""}
  </tr>
</thead>
<tbody>
${noteRow}
`;
  ...
  <td style="vertical-align:top;border:1px solid #e5e7eb;padding:8px;">${r.quantity || 1}</td>
  <td style="vertical-align:top;border:1px solid #e5e7eb;padding:8px;">${r.sku}</td>
  <td style="vertical-align:top;border:1px solid #e5e7eb;padding:8px;">${r.description}</td>
  ${includePrices ? `<td style="vertical-align:top;border:1px solid #e5e7eb;padding:8px;">${fmt(r.msrp)}</td>` : ``}
</tr>`;
      });
      ...
      // Close rich-copy wrapper so only the BOM block is copied with formatting
      html += "</div>";
