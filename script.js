...
function updateA2Qty() {
    ...
    function generate(catalog) {
      ...
      // ===== Guard: PoE injector for A2 Bridge
      (function(){
        const bridge = "B22X2AA#AC3";
        const injector = "A02F9AA";
        if (hasSku(results, bridge) && !hasSku(results, injector)) {
          addLine(results, injector);
        }
      })();
      ...
      if (wantsA2White || wantsA2Black) {
        ...
        if (!v12InBOM) {
          ...
          if (!hasSku(results, "A02F9AA")) addLine(results, "A02F9AA"); // PoE injector for A2 bridge
          if (!hasSku(results, "A02F9AA")) addLine(results, "A02F9AA"); // PoE injector for A2 bridge
