...
function generate(catalog) {
  ...
  const hasSku = (arr, sku) => arr.some(line => line.sku === sku);
  ...
  // === Medium room expansion mic guard (applies to all system types) ===
  if (roomSize === "Medium" && expansionMic === "Yes" && !hasSku(results, "875M6AA")) {
    addLine(results, "875M6AA");
  }
