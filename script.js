...
document.getElementById("expansionMic").addEventListener("change", updateA2Qty);
    function updateCameraVisibility(){
      ...
      const show = (t === "Android appliance based solution" && r === "Very large");
    ...
    function generate(catalog) {
      ...
      if (typeOfSystem === "USB bar only" || typeOfSystem === "Windows PC based solution") {
        ...
        if (typeOfSystem === "Windows PC based solution") {
          if (platform === "Zoom") {
            ...
            // No Meet-imaged PC is provided; do not add any PC here.
            // A BOM NOTE is rendered below to explain this limitation.
          }
        }
      ...
      // ===== Guard rail: ensure Android Very large uses G62 (not X72)
      (function(){
        if (typeOfSystem === "Android appliance based solution" && roomSize === "Very large") {
          // Remove any X72 and its PolyPlus if earlier logic added it
          const toRemove = ["A4MA7AA#ABA","U98SXPV","U98SYPV"];
          for (let i = results.length - 1; i >= 0; i--) {
            if (toRemove.includes(results[i].sku)) results.splice(i,1);
          }
          // Ensure G62 base + PolyPlus present
      ...
      if (implementationHelp && implementationHelp !== "None") {
        let remoteSku = "";
        const hasStudioX = hasSku(results, "A3SV5AA#ABA") || hasSku(results, "8D8L1AA#ABA") || hasSku(results, "A4MA7AA#ABA");
        const hasG62orG7500 = hasSku(results, "A01KCAA#AC3") || hasSku(results, "G62") || hasSku(results, "G7500");
        if (hasStudioX) {
          remoteSku = "PROSTDIOXR2";
        } else if (hasG62orG7500) {
          remoteSku = "PROG7500RE2";
        } else if (typeOfSystem === "USB bar only" || typeOfSystem === "Windows PC based solution") {
