...
function updateA2Qty() {
    ...
    function generate(catalog) {
      ...
      if (wantsA2White || wantsA2Black) {
        ...
        if (!v12InBOM) {
          addLine(results, "B22X2AA#AC3"); if (!hasSku(results, "A02F9AA")) addLine(results, "A02F9AA"); // A2 bridge
      ...
      // ===== Final guard BEFORE render: ensure PoE injector for A2 Bridge is present (qty 1)
      (function(){
        const needInjector = hasSku(results, "B22X2AA#AC3");
        if (!needInjector) return;
        let inj = results.find(r => r.sku === "A02F9AA");
        if (!inj) {
          addLine(results, "A02F9AA");
        } else {
          try { inj.qty = 1; } catch(e) {}
        }
      })();
...
<details class='mt-3'>
  ...
  <div class='mt-2 text-sm'>${refHtml}</div>
      ...
      (function(){
        try {
          if (hasSku(results, "B22X2AA#AC3") && !hasSku(results, "A02F9AA")) {
            addLine(results, "A02F9AA");
          }
        } catch (e) {}
      })();
