...
// Reliable handler for Generate BOM (bound at creation)
...
function updateA2Qty() {
    ...
    document.getElementById("typeOfSystem").addEventListener("change", () => { updatePlatformInfo(); updateExpansionMicOptions(); });
    document.getElementById("roomSize").addEventListener("change", updateExpansionMicOptions);
    document.getElementById("expansionMic").addEventListener("change", updateA2Qty);
    updatePlatformInfo(); updateA2Qty(); updateExpansionMicOptions();
    ...
    function generate(catalog) {
      ...
      (function(){
        const isV12 = hasSku(results, "A9DD8AA#ABA"); // V12 small room bar
        if (!isV12) return;
        if (!hasSku(results, "B5NH6AA")) addLine(results, "B5NH6AA"); // PoE injector for V12
      })();
...
// Safety net: bind by id as well
try { document.getElementById("generateBtn")?.addEventListener("click", () => generate(catalog)); } catch(e) {}
