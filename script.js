...
async function init() {
  ...
  function updateExpansionMicUI(){
    ...
    const allowed = restrict
      ...
      : all;
    // Hide analog option for Very large rooms
    if (r === "Very large") { const idx = allowed.indexOf("Single Analog Exp mic"); if (idx>=0) { allowed.splice(idx,1); } }
  ...
  function generate(){
    ...
    // --- Ensure Analog mic + Extender kit behavior (Medium & Large) ---
