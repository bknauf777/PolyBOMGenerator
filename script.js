...
document.title = 'Poly Video Conferencing "Bill" of Materials Generator v9.16';
async function init() {
  try {
...
if (typeOfSystem === "Android appliance based room system") {
  ...
  // ✅ Expansion mic logic for Android appliance Medium rooms
  if (expansionMic === "Yes") {
    const mic = "875M6AA";
...
if (supportTerm !== "None" && skuDescriptions[supportSku]) {
  results.push({ sku: supportSku, description: skuDescriptions[supportSku] });
}
}
...
if (supportTerm !== "None" && skuDescriptions[supportSku]) {
  ...
  // 🔥 Expansion mic logic for USB bar only Medium/Large rooms
...
if (roomSize === "Small") {
  ...
  // ✅ Expansion mic logic for Windows PC based Medium rooms
  if (expansionMic === "Yes") {
    const mic = "875M6AA";
    if (skuDescriptions[mic]) results.push({ sku: mic, description: skuDescriptions[mic] });
...
// Only add MTR base kit + support if platform is not Google Meet
...
if (platform === "Zoom") {
  ...
  if (skuDescriptions[zoomSku]) results.push({ sku: zoomSku, description: skuDescriptions[zoomSku] });
  if (skuDescriptions[zoomSupport]) results.push({ sku: zoomSupport, description: skuDescriptions[zoomSupport] });
} else {
...
if (supportTerm !== "None" && skuDescriptions[mtrSupport]) {
...
// Expansion mic options table (USB bar only Large rooms)
if (expansionMic === "Yes" && typeOfSystem === "USB bar only" && roomSize === "Large") {
  expansionMicOptions.push({ sku: "875M6AA", description: "Analog expansion mic for Studio USB/X50/X52/V52/X70/X72/V72" });
  expansionMicOptions.push({ sku: "OR", description: "" });
  expansionMicOptions.push({ sku: "874R3AA", description: skuDescriptions["874R3AA"] || "Poly IP table microphone array" });
  expansionMicOptions.push({ sku: "GSM4210PD M4250-9G1F-PoE+", description: "3rd party Netgear AV PoE switch with precision Time protocol (needed if using multiple Poly IP mic arrays)" });
...
if (mounting !== "None") {
  ...
  if (skuDescriptions[panelSupport]) results.push({ sku: panelSupport, description: skuDescriptions[panelSupport] });
  }
...
if (implementationHelp === "Remote Implementation help" || implementationHelp === "Onsite Implementation help") {
  ...
  if (remoteSku) {
    results.push({ sku: remoteSku, description: skuDescriptions[remoteSku] || "(Remote Implementation SKU)" });
  }

  if (implementationHelp === "Onsite Implementation help") {
    results.push({ sku: onsiteSku, description: skuDescriptions[onsiteSku] || "(Onsite Implementation Uplift SKU)" });
  }
}
...
let html = `
  ...
  <p class='text-sm text-blue-700 underline mb-1'>
    <a href="https://docs.google.com/presentation/d/1gIB--U2tGHcWaI1DXk4ovZVH9pme3rUD/edit?usp=sharing&ouid=104449332556651601678&rtpof=true&sd=true" target="_blank">
      For additional Quoting Guide information
    </a>
  </p>

<p class='text-sm text-blue-700 underline mb-1'>
    <a href="https://pcb.inc.hp.com/webapp/#/ww-en/2101689356/M?hierarchy=F&status=L&status=O" target="_blank">
     HP Content browser for Details on skus, What's in the box, etc. To use, search for sku without #ABA #AC3 suffix
    </a>
  </p>

  <p class='text-sm text-blue-700 underline mb-2'>
    <a href="https://www.hp.com/us-en/poly/spaces.html" target="_blank">
      See additional solution wiring diagrams and solution selector with Poly Spaces
    </a>
  </p>
