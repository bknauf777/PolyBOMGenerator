...
function updateA2Qty() {
  ...
  updatePlatformInfo(); updateA2Qty(); updateExpansionMicOptions(); updateCameraVisibility();
    ...
    function generate(catalog) {
      ...
      (function(){
        const isX32 = hasSku(results, "A3SV5AA#ABA"); // Studio X32
        if (!isX32) return;
        if (!hasSku(results, "B5NH6AA")) addLine(results, "B5NH6AA"); // PoE injector
        if (!hasSku(results, "977L6AA#ABA")) addLine(results, "977L6AA#ABA"); // TC10
