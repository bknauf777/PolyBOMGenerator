...
document.title = 'Poly Video Conferencing "Bill" of Materials Generator v9.21';
...
async function init() {
  try {
    ...
    form.innerHTML += `<div id="platformInfo" class="hidden text-sm mt-1 p-2 border-l-4 border-amber-400 bg-amber-50 text-amber-900 rounded">HP Poly does not currently offer a Google Meets imaged PC, but you can use the Poly USB bars along with your own BYOD PC running the regular Meets app, or consider using a Poly Studio X which has the native Google Meets app.</div>`;
...
<input id="accessories" class="border p-2 w-full" placeholder="e.g. Shure P300 DSP, 3rd party powered speaker, extra Poly E60/E70 Camera or mics, Remote control"/></div>`;
    ...
    function updatePlatformInfo() {
      ...
      if (!info) return;
      const shouldShow = (typeOfSystem === "Windows PC based solution" && platform === "Google Meet");
    ...
    document.getElementById("typeOfSystem").addEventListener("change", () => updatePlatformInfo());
...
function generate(catalog) {
  ...
  let bomNoteRow = (typeOfSystem === "Windows PC based solution" && platform === "Google Meet") ? `<tr class='bg-amber-50 text-amber-900'><td class='border px-4 py-2 align-top italic'>NOTE</td><td class='border px-4 py-2 align-top'>HP Poly does not currently offer a Google Meets imaged PC, but you can use the Poly USB bars along with your own BYOD PC running the regular Meets app, or consider using a Poly Studio X which has the native Google Meets app.</td>${includePrices ? `<td class=\'border px-4 py-2 align-top\'>—</td>` : ``}</tr>` : ``;
