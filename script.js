...
const VERSION = "v9.64";
document.title = `Poly Video Conferencing "Bill" of Materials Generator ${VERSION}`;
...
async function init() {
  ...
  function generate(){
    ...
    if (implHelp && implHelp!=="None"){
      let remoteSku = "";
      // Map by system path / device presence
      // Windows PC based OR USB bar only -> PROECOSYS02
      if (typeOfSystem==="Windows PC based solution" || typeOfSystem==="USB bar only") {
        remoteSku = "PROECOSYS02";
      }
      // G62 path -> PROG7500RE2
      else if (hasSku(results,"A01KCAA#AC3")) {
        remoteSku = "PROG7500RE2";
      }
      // Android X-series (X32 / X52 / X72) -> PROSTDIOXR2
      else if (hasSku(results,"A3SV5AA#ABA") || hasSku(results,"8D8K2AA#ABA") || hasSku(results,"A4MA7AA#ABA")) {
    ...
    (function addMounting(){
      if (mounting==="None" || mounting==="") return;
...
/* H1 left static; version shown only in BOM output */
