// script.js v9.13.1 Part 1 – Initialization, form rendering, dependent questions
...
async function init() {
  try {
    ...
    // Event listeners to handle dependent questions
...
// script.js v9.13.1 Part 2 – BOM generation and rendering
...
function generate(skuDescriptions) {
  ...
  // New Very Large room + Android logic
  if (roomSize.includes("Very large") && typeOfSystem === "Android appliance based room system") {
  ...
  // Restored general BOM logic
  if (typeOfSystem === "Windows PC based solution" && platform === "Zoom") {
    const zoomsku = "9D9R3AA"; // Example for small room; adjust your logic for Medium/Large
    if (skuDescriptions[zoomsku]) results.push({ sku: zoomsku, description: skuDescriptions[zoomsku] });
  }
  if (typeOfSystem === "Windows PC based solution" && platform === "Microsoft Teams") {
    const teamssku = "A3LU8AA#ABA";
    if (skuDescriptions[teamssku]) results.push({ sku: teamssku, description: skuDescriptions[teamssku] });
  }

  // Mounting logic (example)
  if (mounting !== "None") {
    const mountsku = "875L6AA"; // Example; implement your correct mapping matrix here
...
// script.js v9.13.1 Part 3 FINAL – Corrected SKU mappings for all system types
